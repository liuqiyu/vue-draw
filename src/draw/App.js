...
App.main = function (callback, container) {
  ...
  function doLoad () {
    ...
    Graph.prototype.defaultThemes['default-style2'] = defaultStyle
    Graph.prototype.defaultThemes['darkTheme'] = darkTheme
  ...
  try {
    if (mxSettings.settings != null) {
      if (mxSettings.settings.autosaveDelay != null) {
        let val = parseInt(mxSettings.settings.autosaveDelay);
      ...
      if (mxSettings.settings.defaultEdgeLength != null) {
        let val = parseInt(mxSettings.settings.defaultEdgeLength);
  ...
  if (urlParams['embed'] != '1') {
    ...
    if (!mxClient.IS_CHROMEAPP && !EditorUi.isElectronApp && urlParams['embed'] != '1' &&
      ...
      mxUtils.get(EditorUi.cacheUrl + '?alive', mxUtils.bind(this, function (req) {
        ...
        if (acceptResponse) {
          EditorUi.logEvent({
            category: 'ALIVE-CACHE-CHECK', action: 'alive', label:
